...
class DataGenerator(object):
    ...
    def _load_notmnist(self):
        ...
        """
        # è il dataset delle informazioni già scaricato in zip
        data = np.load("notMNIST_small.npy", allow_pickle=True)
        data = data.any()

        nsamples = data["images"].shape[0]

        dataset = {} # ti salvi solo le immagini formato nsamples x quante y servono e labels
    ...
    def load(self, name):
        ...
        x_data = data["data"]
        print("data dimension:", x_data.shape)
        y_data = data["target"].reshape(-1,1)
        print("target dimension:", y_data.shape)
        data = np.concatenate((x_data,y_data), axis=-1)
        data = data.astype(np.float32)
        # train_test_split() divide già in X_train, X_test, y_train, y_test very nice!
        trainset, testset = train_test_split(data, test_size=0.25)
